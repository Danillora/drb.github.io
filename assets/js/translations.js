// ============================================================
//  translations.js — drb.github.io
//  Idiomas suportados: en (padrão) | pt
// ============================================================

window.TRANSLATIONS = {
  en: {
    /* ── Navigation ── */
    "nav-home":         "Danillo Rafael Batista",
    "nav-publications": "Publications and Projects",
    "nav-cv":           "CV",
    "nav-contact":      "Contact",

    /* ── Sidebar ── */
    "sidebar-bio":      "Political Science PhD student at UFPE; Visiting Researcher at Boston University",
    "sidebar-location": "Boston - MA, USA",
    "sidebar-follow":   "Follow",

    /* ── Home page ── */
    "home-title": "Hello!",
    "home-body": `
      <p>I'm a PhD student in Political Science at the <a href="https://www.ufpe.br/politica">Graduate Program in Political Science (PPGCP)</a> at the Federal University of Pernambuco (UFPE), currently a Visiting Researcher at Boston University's Department of Political Science, hosted by Prof. Taylor Boas. I hold a Master's in Political Science (Democracy and Institutions) from the same program, as well as a B.A. in International Relations. I also have training in Public Policy Evaluation from the School of Innovation and Public Policies (EIPP/FUNDAJ).</p>

      <p>My research interests include Public Policy Evaluation, Public Opinion, Evangelicals, and Data Analysis. I am a member of the <a href="http://dgp.cnpq.br/dgp/espelhogrupo/45503">Epistemology and Method in Comparative Political Science</a> group (led by Prof. Flávio Rezende) and the <a href="https://laberpfundajufpe.medium.com/">Laboratory of Religion and Politics Studies</a> (led by Prof. Joanildo Burity).</p>

      <p>Additionally, I collaborate with the <a href="https://x.com/metodosempauta">Métodos em Pauta initiative</a>, which aims to foster discussions on scientific methodology and inferential quality in Social Sciences. I also run <a href="https://brewed-evidence.quarto.pub/home/">Brewed Evidence</a>, a blog where I write about academic things and other interests. The project is still in its early stages.</p>

      <p>For more details, you can check my academic CV (Lattes), repositories, and social networks in the menu on the left side of the page.</p>
    `,

    /* ── Publications page ── */
    "pub-title": "Publications and Projects",
    "pub-section-ongoing": "Working Papers",

    /* ── CV page ── */
    "cv-title": "CV",
    "cv-education": "Education",
    "cv-visiting": "Visiting Research",
    "cv-work": "Work experience",
    "cv-skills": "Skills",
    "cv-presentations": "Presentations and Conferences",
    "cv-groups": "Research Groups Enrolled",
    "cv-teaching": "Teaching",
    "cv-scholarships": "Scholarships",
    "cv-additional-formation": "Additional Formation",
    "cv-additional-info": "Additional Information",
    "cv-current-position": "Current Position",
    "cv-previous-position": "Previous Position",

    /* ── Contact page ── */
    "contact-title": "Contact",

    /* ── Footer ── */
    "footer-powered": "Powered by",
    "lang-button-label": "🇧🇷 PT"
  },

  pt: {
    /* ── Navigation ── */
    "nav-home":         "Danillo Rafael Batista",
    "nav-publications": "Publicações e Projetos",
    "nav-cv":           "Currículo",
    "nav-contact":      "Contato",

    /* ── Sidebar ── */
    "sidebar-bio":      "Doutorando em Ciência Política na UFPE; Pesquisador Visitante na Boston University",
    "sidebar-location": "Boston - MA, EUA",
    "sidebar-follow":   "Seguir",

    /* ── Home page ── */
    "home-title": "Olá!",
    "home-body": `
      <p>Sou doutorando em Ciência Política no <a href="https://www.ufpe.br/politica">Programa de Pós-Graduação em Ciência Política (PPGCP)</a> da Universidade Federal de Pernambuco (UFPE), atualmente Pesquisador Visitante no Departamento de Ciência Política da Boston University, sob orientação do Prof. Taylor Boas. Sou mestre em Ciência Política (Democracia e Instituições) pela mesma instituição e bacharel em Relações Internacionais. Também possuo formação em Avaliação de Políticas Públicas pela Escola de Inovação e Políticas Públicas (EIPP/FUNDAJ).</p>

      <p>Meus interesses de pesquisa incluem Avaliação de Políticas Públicas, Opinião Pública, Evangélicos e Análise de Dados. Sou membro do grupo <a href="http://dgp.cnpq.br/dgp/espelhogrupo/45503">Epistemologia e Método em Ciência Política Comparada</a> (coordenado pelo Prof. Flávio Rezende) e do <a href="https://laberpfundajufpe.medium.com/">Laboratório de Estudos de Religião e Política (LABERP)</a> (coordenado pelo Prof. Joanildo Burity).</p>

      <p>Também colaboro com a <a href="https://x.com/metodosempauta">iniciativa Métodos em Pauta</a>, que visa fomentar discussões sobre metodologia científica e qualidade inferencial nas Ciências Sociais. Além disso, mantenho o <a href="https://brewed-evidence.quarto.pub/home/">Brewed Evidence</a>, um blog onde escrevo sobre temas acadêmicos e outros interesses. O projeto ainda está em seus estágios iniciais.</p>

      <p>Para mais detalhes, consulte meu currículo acadêmico (Lattes), repositórios e redes sociais no menu à esquerda da página.</p>
    `,

    /* ── Publications page ── */
    "pub-title": "Publicações e Projetos",
    "pub-section-ongoing": "Working Papers",

    /* ── CV page ── */
    "cv-title": "Currículo",
    "cv-education": "Formação Acadêmica",
    "cv-visiting": "Pesquisa Visitante",
    "cv-work": "Experiência Profissional",
    "cv-skills": "Competências",
    "cv-presentations": "Apresentações e Conferências",
    "cv-groups": "Grupos de Pesquisa",
    "cv-teaching": "Ensino",
    "cv-scholarships": "Bolsas e Financiamentos",
    "cv-additional-formation": "Formação Complementar",
    "cv-additional-info": "Informações Adicionais",
    "cv-current-position": "Cargo Atual",
    "cv-previous-position": "Cargo Anterior",

    /* ── Contact page ── */
    "contact-title": "Contato",

    /* ── Footer ── */
    "footer-powered": "Desenvolvido com",
    "lang-button-label": "🇺🇸 EN"
  }
};
